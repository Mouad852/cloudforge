...
3. **`[PENDING]` EC2 vCPU quota increase (8 → 16, `eu-west-3`).** Hit the account's 8 vCPU limit for real during M3, when an ASG instance refresh needed one extra instance alongside the existing two to keep `min_healthy_percentage = 100` — I had to manually stop the unrelated `ssm_test` instance to free capacity. My first request through Service Quotas was auto-generated with no use case at all and got denied outright. I reopened it with the actual story above, plus the fact that M8's blue/green deploy will need similar temporary headroom, and I'm waiting on AWS's re-assessment. Not blocking M4 — CloudFront/WAF/ALB don't touch this quota at all — but it does gate how much room M8 has to work with.
