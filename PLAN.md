...
3. **`[RESOLVED]` EC2 vCPU quota increase (8 → 16, `eu-west-3`).** Hit the account's 8 vCPU limit for real during M3, when an ASG instance refresh needed one extra instance alongside the existing two to keep `min_healthy_percentage = 100` — I had to manually stop the unrelated `ssm_test` instance to free capacity. My first request through Service Quotas was auto-generated with no use case at all and got denied outright. I reopened it with the actual story above, plus the fact that M8's blue/green deploy will need similar temporary headroom. AWS approved the appeal — confirmed in the Service Quotas console, `L-1216C47A` now shows an applied account-level quota value of 16 (up from the AWS default of 5), against a current utilization of 8. Gives M8's blue/green deploy the headroom it'll need alongside the existing ASG capacity.
4. **`[PENDING]` CloudFront account verification.** M4's `aws_cloudfront_distribution` apply failed with `AccessDenied: Your account must be verified before you can add new CloudFront resources`. Confirmed it's an account-trust restriction, not a config problem — every other M4 resource (WAF web ACL, ALB, target groups) created fine in the same apply, only the CloudFront resource itself was rejected. I opened a support case under Account and billing → Account describing the project and pasting the exact error. M4's Terraform is fully written and applied everywhere except this one resource; the milestone's DoD (a working `https://<dist>.cloudfront.net` URL) can't be proven until this clears.
