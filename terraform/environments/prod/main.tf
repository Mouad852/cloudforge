...
variable "snapshot_identifier" {
  description = "Restore prod-cloudforge-db from this snapshot instead of creating an empty one - set by `make prod-up` after a `make prod-down` (ADR-015)"
  type        = string
  default     = null
}
...
module "compute" {
  ...
  # Same capacity finding as dev (see dev/main.tf) - t4g.micro had no
  # capacity in eu-west-3a/3b when this was tested; t4g.small proven
  # end-to-end there, applied here for the same reason.
...
module "database" {
  ...
  # Real production settings - dev leaves both false so a throwaway dev DB
  # can actually be destroyed without a standby replica it doesn't need.
  multi_az            = true
