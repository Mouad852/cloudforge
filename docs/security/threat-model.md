...
**The redesign is applied to `dev` and `prod` and was verified with real requests on 2026-09-24/25** (results in `docs/diagrams/traffic-flow.md` and `data-flow.md`). One exception: the SQL-injection rule group (T2) was added to the code on 2026-09-25, after that testing found the WAF had none, and is not yet applied.
...
| WAF on the ALB | `REGIONAL`-scope web ACL, associated directly with the ALB (`aws_wafv2_web_acl_association`), default action allow. Rules in order: `AWSManagedRulesCommonRuleSet`, `AWSManagedRulesKnownBadInputsRuleSet`, `AWSManagedRulesAmazonIpReputationList`, `RateLimitPerIP` (block above `waf_rate_limit`, default 2000 requests per 5 minutes per IP), then `AWSManagedRulesSQLiRuleSet` (added 2026-09-25, see T2). Metrics and sampled requests on; no request logging. | `modules/edge` |
...
- [ ] Once the SQLi rule group is applied, the CloudShell test request from T2 gets a `403`, and the sampled requests name `AWSManagedRulesSQLiRuleSet` (T2). Send it from CloudShell, not the operator's own network (see `docs/diagrams/traffic-flow.md`).
