...
| WAF on the ALB | `REGIONAL`-scope web ACL, associated directly with the ALB (`aws_wafv2_web_acl_association`), default action allow. Rules in order: `AWSManagedRulesCommonRuleSet`, `AWSManagedRulesKnownBadInputsRuleSet`, `AWSManagedRulesAmazonIpReputationList`, then `RateLimitPerIP` (block above `waf_rate_limit`, default 2000 requests per 5 minutes per IP). Metrics and sampled requests on; no request logging. | `modules/edge` |
...
| T2 | X1 sends known-bad web payloads (SQL injection strings, scanner traffic, known bad IPs). | WAF Common, KnownBadInputs and IP Reputation rule groups block before the ALB forwards the request. | Managed rules do not stop logic abuse (T1). No bot control. | Accepted |
